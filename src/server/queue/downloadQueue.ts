...
const downloadWorker = new Worker<DownloadJobData>(
  ...
  async (job: Job<DownloadJobData>): Promise<DownloadResult> => {
    ...
    try {
      ...
      function processSpotifySongName(songArtists: string[], songTitle: string): [string[], string] {
        ...
        for (const featString of featStrings) {
          if (songTitle.toLowerCase().includes(featString.toLowerCase())) {
            const regex = new RegExp(featString.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
        ...
        for (const artist of artists) {
          if (!songArtists.some(existingArtist => 
      ...
      const initialArtists = allArtists && allArtists.length > 0 ? [...allArtists] : [artistName];
      const [finalArtists, processedTitle] = processSpotifySongName(initialArtists, trackName);

      // Create filename with all artists
      const safeTrackName = processedTitle.replace(/:/g, '').replace(/&/g, 'and').trim();
      ...
      try {
        await exec(`yt-dlp --no-download --print-json "${downloadUrl}"`, { timeout: 30000 });
      } catch (infoError) {
        const errorMessage = infoError instanceof Error ? infoError.message : String(infoError);
        throw new Error(`Video unavailable or yt-dlp not found: ${errorMessage}`);
      }
      ...
      const tempFilePath = join(tempDir, `download-${videoId}-${Date.now()}.mp3`);
      ...
      const downloadPromise = new Promise<void>((resolve, reject) => {
        ...
        ytDlpProcess.on('close', (code) => {
          console.log(`yt-dlp process exited with code ${code} for ${trackName}`);
          cleanup();
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        downloadTimeout = setTimeout(() => {
          console.error(`⏱️ Download timeout for ${trackName} after 1 minute 30 seconds`);
          ytDlpProcess.kill('SIGTERM');
          reject(new Error(`Download timeout after 1 minute 30 seconds - ${trackName} may be unavailable or too large`));
      ...
      console.log(`Download completed for ${trackName}, file size: ${totalSize} bytes`);
      ...
      await redis.setex(DOWNLOAD_FILE_KEY(downloadId), 3600, JSON.stringify(fileData)); // 1 hour TTL

      console.log(`✅ Download completed and stored in Redis: ${downloadId}, size: ${totalSize} bytes`);
      ...
      console.error(`❌ Download failed for ${videoId} (${trackName}):`, errorMessage);
...
downloadWorker.on('completed', (job, result) => {
  ...
  console.log(`✅ Download completed: ${job.data.trackName} by ${job.data.artistName} (${downloadResult?.fileSize ?? 0} bytes)`);
});

downloadWorker.on('failed', (job, err) => {
  console.error(`❌ Job FAILED: ${job?.data?.trackName} by ${job?.data?.artistName} - ${err?.message ?? err}`);
});
...
downloadWorker.on('progress', (job, progress) => {
  ...
  if (progressNum > 0) {
    console.log(`📊 Download progress for ${job.data.trackName}: ${progressNum.toFixed(1)}%`);
  }
});
