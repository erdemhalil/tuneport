...
} from "~/server/queue/downloadQueue";
...
function calculateDurationConfidence(
...
function isYouTubeContentExplicit(
  title: string,
  channel: string,
  madeForKids?: boolean,
...
export const youtubeRouter = createTRPCRouter({
  searchYouTube: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!detailsResponse.ok) {
        ...
        throw new Error(
          `YouTube Videos API error: ${detailsResponse.status} - ${errorText}`,
        );
      }
  ...
  downloadTracks: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      console.log(
        `🎵 downloadTracks called for user ${userId} with ${input.tracks.length} tracks`,
      );
