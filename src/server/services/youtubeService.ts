...
export class YouTubeService {
  ...
  async search(
    ...
    const cacheKey = `youtube:search:${searchQuery}:${input.durationMs}:${maxResults}`;

    return this.cachedFetch(
      cacheKey,
      async () => {
        const searchData = await this.fetchYouTubeApi(
          `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(searchQuery)}&type=video&maxResults=${maxResults}`,
          "Search",
